...
const Card = ({ data }) => {
  ...
  return (
    <motion.div
      className="relative w-full bg-neutral-900 px-5 py-5 rounded-lg border border-neutral-700 overflow-hidden group"
      onMouseMove={(event) => {
    ...
    >
      ...
      <div className="relative flex flex-wrap gap-x-2 gap-y-3 text-sm items-start mt-3 z-10">
        {data.skills.map((tech, index) => (
          <p
            key={index}
            className="py-1 px-3 bg-neutral-800 border border-neutral-600 rounded-full text-white"
          >
            {tech}
          </p>
        ))}
